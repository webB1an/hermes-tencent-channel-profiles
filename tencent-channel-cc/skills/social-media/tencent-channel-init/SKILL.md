...
tencent-channel-cli 相关脚本的路径约定（profile: `tencent-channel-june`）：

| 变量 | 默认路径 |
|------|---------|
| `HERMES_HOME` | `/root/.hermes/profiles/tencent-channel-june` |
| `WALLPAPER_DIR` | `/root/.hermes/profiles/tencent-channel-june/media` |
| `STATE_FILE` | `/root/.hermes/profiles/tencent-channel-june/wallpaper_state.json` |

`.env` 文件位置：`<HERMES_HOME>/.qqcli/.env`
