...
2. **短贴 vs 长贴**：正文 ≤1000 加权字 → 短贴(feed_type=1)无需标题；>1000 → 长贴(feed_type=2)需标题。用户要求长贴时先索取标题。禁止未提供 title 时擅自用 feed_type=2。**加权字**：中文/中文标点=1字，英文/数字/半角=0.5字
3. **数量限制**：短贴 ≤1000字/≤18图/≤1视频；长贴 ≤10000字/≤50图/≤5视频；评论回复 ≤1图。超限 CLI 直接报错
4. **超限拆分**：严禁自行拆分发布，必须先告知用户 → 提出拆分方案 → 获得确认后执行
5. 正文为纯文本，不渲染 Markdown
6. 成功后自动补取分享短链，展示短链即可，**不返回 feed_id**
