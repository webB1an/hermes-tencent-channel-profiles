...
```
/root/.hermes/profiles/tencent-channel-live-wallpaper/scripts/live_wallpaper_post.py
```

**使用方式：**
```bash
# 完整流程（下载 → 发帖 → 删除）
python3 scripts/live_wallpaper_post.py

# 预演（下载但不发帖）
python3 scripts/live_wallpaper_post.py --dry-run

# 详细日志
python3 scripts/live_wallpaper_post.py -v
```
...
### 22. Page 1 并非完全耗尽——源站持续更新

~~截至 2026-05-18，Page 1 三个源已全部覆盖：~~

**实际观察（2026-05-18）**：WallpaperWaifu Page 1 **仍有新内容**，Yae Miko Sleeping 在 dedup 中不存在，成功下载并发帖（88.3MB）。说明 WallpaperWaifu 源站会持续更新 Page 1，并非一次性耗尽。

- **WallpaperWaifu Page 1**：22 条壁纸，**部分**在 `posted_detail_urls` 中，dedup 检查后仍有新内容
- **MoeWalls Page 1**：15 条壁纸，全部已记录
- **DesktopHut**：网络超时（12s），服务器 IP 被墙

**当前 dedup 规模**：`live_wallpaper_state.json` 记录 `posted_detail_urls` 64 条（+1 今日 Yae Miko），`posted_urls` 0 条。

**策略**：每次触发都重新扫描 WallpaperWaifu Page 1，不要假设"首页已耗尽"。WallpaperWaifu 各分类 Page 2+ 仍有 ~44 条未覆盖（anime/games/fantasy/landscape/lifestyle/movies/pixel-art）可作为扩展来源。

### 23. 超时后残留锁文件的手动清理

`live_wallpaper_post.py` 有进程锁（`flcntl.flock`），超时被 kill 后锁文件 `/tmp/live_wallpaper_post.lock` 可能残留（大小 0）。下次运行时 flock 会失败并立即退出。

**症状**：脚本启动后日志输出"===== 动态壁纸发帖开始 @" 后立即卡住约 120s，然后超时退出；或直接无进一步输出而 exit code 124。lock 文件存在时后续实例均无法启动。
...
**注意**：2026-05-18 实测，第一次触发时超时（120s），日志已有部分输出（WallpaperWaifu 下载成功），说明脚本在超时过程中仍在运行，只是下载+发帖总时间超过 120s 阈值被外层 timeout kill 了。锁文件残留是 kill 导致的，不是脚本 bug。
...
| state 文件 | `/root/.hermes/profiles/tencent-channel-live-wallpaper/live_wallpaper_state.json` |
| 下载目录 | `scripts/live-wallpaper-download/downloads/` |
